...
[<FunctionName("EscalationLvlLow")>]
let Run([<QueueTrigger(EscalationLvlLow)>] content : string, log : ILogger) =
    task {
        let mail = JsonConvert.DeserializeObject<MailContent>(content)
        let from = MailAddress(user, "EscalationLvlLow")
