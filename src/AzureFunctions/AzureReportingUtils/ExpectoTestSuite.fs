...
let fileWriterInfo = initFileWriter Development "PreveroFeed" Local
let fileWriterInfoAzure = initFileWriter Development "PreveroFeed" Azure

[<FunctionName("ExpectoTestSuite")>]
let Run([<TimerTrigger("0 0 0 1 * *")>] myTimer : TimerInfo, log : ILogger) =
    task {
        let! weatherData = getWeatherData (weather fileWriterInfo)
        let testList =
